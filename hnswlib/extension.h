...
HierarchicalNSW<dist_t> *HNSWMerger_BS(HierarchicalNSW<dist_t> *index1, HierarchicalNSW<dist_t> *index2, L2Space *space, int lambda = 4, float alpha = 1.05, size_t M = -1, size_t ef_construction = -1) {
    ...
    if (layer_node_for_index1[maxLevel].size() + layer_node_for_index2[maxLevel].size() > M) {
        ...
        filter_and_remove(layer_node_for_index2[maxLevel], index2,
                          index2_offset);
        // TODO: fixed element_level issue and distance computation/data load, for multiindex;
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (size_t i = 0; i < indices.size(); i++) {
        ...
        indices[i]->searchNodeOnEachLayer(layer_nodes_for_indices[i], true);
    ...
    for (int level = maxLevel; level >= 0; --level) {
        ...
        if (count_indices_with_nodes == 1) {
            merged_index->deepCopyOneLayerOnIndex(
                ...
                level,
                index_offsets[index_with_nodes],
                layer_nodes_for_indices[index_with_nodes]);
            continue;
        }
        if (count_indices_with_nodes == 0) continue;

        std::vector<std::vector<std::pair<dist_t, tableint>>> candidateSetsForNode(total_elements);
        std::vector<std::mutex> mtx(total_elements);
        size_t Mcurmax = level ? merged_index->maxM_ : merged_index->maxM0_;
        int lambda = 4;
        int index_size = indices.size();

        for (size_t idx = 0; idx < indices.size(); ++idx) {
            auto &points = layer_nodes_for_indices[idx][level];
#pragma omp parallel for
            for (size_t xi = 0; xi < points.size(); ++xi) {
                ...
                int lowerLevel = level;
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, typename HierarchicalNSW<dist_t>::CompareByFirst> candidates;

                int highBound;
                if (index_size % 2 || idx % 2)
                    highBound = (int)(index_size / 2);
                else
                    highBound = (int)(index_size / 2) - 1;
                for (size_t target_idx_id = 1; target_idx_id <= highBound; ++target_idx_id) { // TODO: fix or not
                    int target_idx = (idx + target_idx_id) % index_size;
                    ...
                    while (!top_candidates.empty()) {
                        ...
                        // candidates.emplace(current);
                        std::lock_guard<std::mutex> lk1(mtx[source_node]);
                        candidateSetsForNode[source_node].push_back(current);

                        std::lock_guard<std::mutex> lk2(mtx[raw_target]);
                        candidateSetsForNode[raw_target].push_back(std::make_pair(current.first, source_node));
                    }
                }
            }
        }
...
#pragma omp parallel for
            for (size_t xi = 0; xi < points.size(); ++xi) {
                ...
                char *data_point = indices[idx]->getDataByInternalId(cur_c);
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, typename HierarchicalNSW<dist_t>::CompareByFirst> candidates;
                if (candidateSetsForNode[source_node].size() + indices[idx]->getListCount(indices[idx]->get_linklist_at_level(cur_c, level)) > Mcurmax) {
                    ...
                    for (size_t i = 0; i < linklistCount; ++i) {
                        candidates.emplace(dist[i], data[i] + index_offsets[idx]);
                    }
                    for (size_t i = 0; i < candidateSetsForNode[source_node].size(); i++) {
                        candidates.emplace(candidateSetsForNode[source_node][i]);
                    }
                    merged_index->getNeighborsByHeuristic2(
                        candidates, Mcurmax, false, -1, 1.05);

                    ll_cur = merged_index->get_linklist_at_level(source_node, level);
                    merged_index->setListCount(ll_cur, candidates.size());
                    auto new_data = reinterpret_cast<tableint *>(ll_cur + 1);
                    auto new_dist = reinterpret_cast<dist_t *>(merged_index->get_dist_at_level(source_node, level));
                    for (size_t i = 0; !candidates.empty(); ++i) {
                        new_data[i] = candidates.top().second;
                        new_dist[i] = candidates.top().first;
                        candidates.pop();
                    }
                } else {
                    linklistsizeint *ll_cur_index1 = indices[idx]->get_linklist_at_level(cur_c, level);
                    size_t linklistCount_index1 = indices[idx]->getListCount(ll_cur_index1);
                    tableint *data_index1 = (tableint *)(ll_cur_index1 + 1);
                    dist_t *dist_index1 = indices[idx]->get_dist_at_level(cur_c, level);

                    auto ll_cur = merged_index->get_linklist_at_level(cur_c + index_offsets[idx], level);
                    merged_index->setListCount(ll_cur, linklistCount_index1 + candidateSetsForNode[source_node].size());
                    auto data = (tableint *)(ll_cur + 1);
                    dist_t *distData = (dist_t *)merged_index->get_dist_at_level(cur_c + index_offsets[idx], level);
                    size_t offset = 0;
                    for (size_t i = 0; i < linklistCount_index1; i++) {
                        data[offset] = data_index1[i] + index_offsets[idx];
                        distData[offset++] = dist_index1[i];
                    }
                    for (size_t i = 0; i < candidateSetsForNode[source_node].size(); i++) {
                        data[offset] = candidateSetsForNode[source_node][i].second;
                        distData[offset++] = candidateSetsForNode[source_node][i].first;
                    }
                }
            }
        }
        ...
        //                 // lowerLevel = (level == 0 && indices[idx]->element_levels_[cur_c] == 0) ? 1 : level;
        //                 lowerLevel = level;
        //                 std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, typename HierarchicalNSW<dist_t>::CompareByFirst> candidates;

        //                 // for (size_t target_idx = idx + 1; target_idx < indices.size(); ++target_idx) { // TODO: fix or not
        //                 for (size_t target_idx = 0; target_idx < indices.size(); ++target_idx) { // TODO: fix or not
        //                     if (target_idx == idx)
        //                         continue;
        //                     auto &last_entry_point = entry_points_collect[idx][target_idx][cur_c];
        //                     int higherLevel = (last_entry_point == -1) ? indices[target_idx]->maxlevel_ : level;
        ...
        //                         // if (level > 0) {
        //                             std::lock_guard<std::mutex> lk(mtx[raw_target]);
        //                             candidateSetsForNode[raw_target].push_back(std::make_pair(current.first, source_node));
        //                         // }
        //                     }
        //                 }
        //                 if (candidates.size() + candidateSetsForNode[source_node].size() + indices[idx]->getListCount(indices[idx]->get_linklist_at_level(cur_c, level)) > Mcurmax) {
        //                     auto ll_cur = indices[idx]->get_linklist_at_level(cur_c, level);
        //                     size_t linklistCount = indices[idx]->getListCount(ll_cur);
        //                     auto data = (tableint *)(ll_cur + 1);
        //                     auto dist = indices[idx]->get_dist_at_level(cur_c, level);
        ...
        //                     auto ll_cur = merged_index->get_linklist_at_level(cur_c + index_offsets[idx], level);
        //                     merged_index->setListCount(ll_cur, candidates.size() + linklistCount_index1 + candidateSetsForNode[source_node].size());
        //                     auto data = (tableint *)(ll_cur + 1);
        //                     dist_t *distData = (dist_t *)merged_index->get_dist_at_level(cur_c + index_offsets[idx], level);
        //                     size_t offset = 0;
        //                     for (size_t i = 0; candidates.size() > 0; i++) {
        //                         data[offset] = candidates.top().second;
        //                         distData[offset++] = candidates.top().first;
        //                         candidates.pop();
        //                     }
        //                     for (size_t i = 0; i < linklistCount_index1; i++) {
        //                         data[offset] = data_index1[i] + index_offsets[idx];
