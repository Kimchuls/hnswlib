...
void create_index() {
...

    {
        ...
        //   // }
        // }
        ParallelFor(0.5e6, 1e6, 80, [&](size_t row, size_t threadId) { alg_hnsw0->addPoint((void *)(xb + dim * row), row); });
        printf("[%.3f s] build index0\n", elapsed() - t0);
        ...
        alg_hnsw0->saveIndex("/ssd_root/jin467/merger/indexes/test/bigann-index_500K-1M.hnsw");
