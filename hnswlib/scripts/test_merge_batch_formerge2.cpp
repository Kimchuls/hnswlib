...
int main()
{
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    {
      alg_hnsw_array[i]->addPoint(xb + (100000 * i + j) * dim, 100000 * i + j);
    }
  }
  t01 = elapsed() - t0;
  ...
  t0 = elapsed();
  hnswlib::HierarchicalNSW<float> *merge1 = hnswlib::HNSWMergerOnMultiple<float>(alg_hnsw_array, 10, &space);
  // for (int i = 2; i < 10; i++)
  // {
  //   merge1 = hnswlib::HNSWMerger<float>(merge1, alg_hnsw_array[i], &space);
  // }
  merge1->setEf(200);
  t01 = elapsed() - t0;
  ...
  // t0 = elapsed();
  // hnswlib::HierarchicalNSW<float> *merge3 = hnswlib::HNSWMerger<float>(alg_hnsw_array[0], alg_hnsw_array[1], &space);
  // hnswlib::HierarchicalNSW<float> *merge4 = hnswlib::HNSWMerger<float>(alg_hnsw_array[2], alg_hnsw_array[3], &space);
  // hnswlib::HierarchicalNSW<float> *merge5 = hnswlib::HNSWMerger<float>(alg_hnsw_array[4], alg_hnsw_array[5], &space);
  // hnswlib::HierarchicalNSW<float> *merge6 = hnswlib::HNSWMerger<float>(alg_hnsw_array[6], alg_hnsw_array[7], &space);
  // hnswlib::HierarchicalNSW<float> *merge7 = hnswlib::HNSWMerger<float>(alg_hnsw_array[8], alg_hnsw_array[9], &space);
  // hnswlib::HierarchicalNSW<float> *merge8 = hnswlib::HNSWMerger<float>(merge3, merge4, &space);
  // hnswlib::HierarchicalNSW<float> *merge9 = hnswlib::HNSWMerger<float>(merge5, merge6, &space);
  // hnswlib::HierarchicalNSW<float> *merge10 = hnswlib::HNSWMerger<float>(merge7, merge8, &space);
  // hnswlib::HierarchicalNSW<float> *merge2 = hnswlib::HNSWMerger<float>(merge9, merge10, &space);
  // merge2->setEf(200);
  // t01 = elapsed() - t0;
  // printf("[%.3f s] build merged index binary\n", t01);
