...
int main()
{
  ...
  hnswlib::HierarchicalNSW<float> *alg_hnsw20M = new hnswlib::HierarchicalNSW<float>(&space, 500000, M, ef_construction);
  ...
  int listing[] = {500000, 1000000, 2000000, 5000000, 10000000};
  ...
  // }
  // alg_hnsw20M->saveIndex("./indexes/bigann-index_500K.hnsw");
  alg_hnsw20M->loadIndex("./indexes/bigann-index_500K.hnsw", &space);
  t01 = elapsed() - t0;
  ...
  for (int iter = 1; iter < 5; iter++)
  {
...

    {
      ...
      // }
      // alg_hnsw1->saveIndex("./indexes/bigann-index_" + std::to_string(listing[iter-1]) +"_"+ std::to_string(listing[iter])+".hnsw");
      alg_hnsw1->loadIndex("./indexes/bigann-index_" + std::to_string(listing[iter-1]) +"_"+ std::to_string(listing[iter])+ ".hnsw", &space);
      t01 = elapsed() - t0;
...

    {
      ...
      // alg_hnsw20M->saveIndex("bigann-index" + std::to_string(listing[iter]) + ".hnsw");
    }
